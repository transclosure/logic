...
// Scoping Algorithm
one sig Scope {
	discoveries: Condition -> set Condition, -- full discovery (not just from goal)
	discovered: set Condition,	-- discovered conditions from goal
	...
	-- fix underconstraint
	all a:Action | no a.targets[Condition-a.precond]
	E not in discoveries[Condition]
	-- scope task
	discoveries[E] = GoalP7X+GoalP7Y+GoalP7I
	-- process conditions WORKS FOR NON-MINIMAL MODELS
	all d,p:Condition-E | (p in discoveries[d]) iff (some a:Action | (some a.targets[p] & d.vars))
	discovered = E.^discoveries
	{d:discovered | (d.val=True)} = causallinks
	all a:Action | (a in usedactions) iff (some d,p:discovered | (some a.targets[d] & p.vars))
	-- find threats
	all c:Condition | (c in brokenlinks) iff (c in causallinks and (some a:usedactions | some a.targets[a.precond] & c.vars))
	-- relevant conditions and variables
	relevant = (discovered - causallinks + brokenlinks).vars
}
run {} for 10 Int
